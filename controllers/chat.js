...
import {ALERT,REFETCH_CHATS,NEW_MESSAGE,NEW_MESSAGE_ALERT} from "../constants/event.js"
...
const newGroupChat = TryCatch(async (req, res, next) => {
  ...
  const getMyChats=TryCatch(async(req,res,next)=>{
    const chats=await Chat.find({members:req.user}).populate("members","name avatar");
     ...
     const tranformedChats=await chats.map(({_id,name,members,groupChat})=>{
...
return res.status(201).json({
  ...
  const renameGroup=TryCatch(async(req,res,next)=>{
    ...
    if(chat.creator.toString()!==req.user.toString()){
      return next(new ErrorHandler("you have no authority to change the group name",403))
    }
...
export {newGroupChat,getMyChats,getMyGroups
  ...
  sendAttachments,getChatDetails,renameGroup,deleteChat
,getMessages
}